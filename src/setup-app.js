(function () {
  ...
  function renderAuth(groups) {
    ...
    if (!advancedToggle) {
      ...
      advancedToggle.innerHTML = '<input type="checkbox" id="showAdvancedAuth" /> Show interactive OAuth options';
  ...
  function refreshStatus() {
    ...
    return httpJson('/setup/api/status').then(function (j) {
      ...
      if (statusDetailsEl) {
        statusDetailsEl.textContent = 'Gateway target: ' + (j.gatewayTarget || '(unknown)');
      }
  ...
  function loadAuthGroupsFast() {
    return httpJson('/setup/api/auth-groups').then(function (j) {
      ...
      console.warn('[atlas] authGroups load failed:', e);
      renderAuth([]);
    });
  }
  ...
  if (pairingBtn) {
    pairingBtn.onclick = function () {
      var channel = prompt('Enter channel (telegram):');
      if (!channel) return;
      channel = channel.trim().toLowerCase();
      if (channel !== 'telegram') { alert('Channel must be "telegram"'); return; }
      var code = prompt('Enter pairing code:');
      if (!code) return;
  ...
  function refreshDevices() {
    ...
    return httpJson('/setup/api/devices/pending').then(function (j) {
      ...
      if (!ids.length) { devicesListEl.textContent = 'No pending device requests.'; return; }
